...
class HttpResponse:
    def __init__(self, response=""):
        """
        Creates a HttpResponse with default properties, unless a response message is provided
        ...
        if response:
            ...
            if 'status_code' in header: self.status_code = header['status_code']
            if 'reason' in header: self.reason = header['reason']
            if 'connection' in header: self.connection = header['connection']
            if 'date' in header: self.date = header['date']
    ...
    def gen_response(self):
        ...
        if self.content: req_bytes.extend(self.content)
